...
void CDemoCamera::GenerateSyntheticImage(ImgBuffer& img, double exp)
{
   ...
   if (pixelType.compare(g_PixelType_8bit) == 0)
   {
      ...
      for (j=0; j<img.Height(); j++)
      {
         for (k=0; k<imgWidth; k++)
         {
            ...
            unsigned char val = (unsigned char) (g_IntensityFactor * std::min(255.0, (pedestal + dAmp * sin(dPhase_ + dLinePhase + (2.0 * lSinePeriod * k) / lPeriod))));
            if (val > maxDrawnVal) {
                maxDrawnVal = val;
            }
            *(pBuf + lIndex) = val;
   ...
   else if (pixelType.compare(g_PixelType_16bit) == 0)
   {
      ...
      for (j=0; j<img.Height(); j++)
      {
         ...
         {
            long lIndex = imgWidth*j + k;
            unsigned short val = (unsigned short) (g_IntensityFactor * std::min((double)maxValue, pedestal + dAmp16 * sin(dPhase_ + dLinePhase + (2.0 * lSinePeriod * k) / lPeriod)));
            if (val > maxDrawnVal) {
                maxDrawnVal = val;
            }
            *(pBuf + lIndex) = val;
   ...
   else if (pixelType.compare(g_PixelType_32bit) == 0)
   {
      ...
      for (j=0; j<img.Height(); j++)
      {
         ...
         {
            long lIndex = imgWidth*j + k;
            double value =  (g_IntensityFactor * std::min(255.0, (pedestal + dAmp * sin(dPhase_ + dLinePhase + (2.0 * lSinePeriod * k) / lPeriod))));
